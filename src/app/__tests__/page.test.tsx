...
describe('Home Page', () => {
  ...
  it('renders the main heading and form', () => {
    ...
    expect(screen.getByRole('button', { name: /Get My Humanity Passport/ })).toBeInTheDocument();
  });
  ...
  it('has proper form structure', () => {
    ...
    const form = screen.getByRole('button', { name: /Get My Humanity Passport/ }).closest('form');
    expect(form).toBeInTheDocument();
    
    const input = screen.getByLabelText(/GitHub Repository URL/);
    expect(input).toHaveAttribute('type', 'url');
    expect(input).toHaveAttribute('placeholder', 'https://github.com/owner/repository');
  });
});